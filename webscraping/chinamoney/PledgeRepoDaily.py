...
DB_NAME = 'chinamoney'
TABLE_NAME = 'PledgeRepoDaily'

url = "http://www.chinamoney.com.cn/fe-c/PledgeRepoDailySearchAction.do"
...
def insert_db_sql_values():
    # 引号坑死人.字段内不能加特殊符号 比如%
    sql = "insert into PledgeRepoDaily (date,`品种`,`开盘利率`,`收盘利率`,`加权利率`, `升降(基点)`," \
          "`加权平均利率(利率债)`,`平均拆借期限(天)`,`成交笔数(笔)`,`成交金额(亿元)`,`增减(亿元)`)" \

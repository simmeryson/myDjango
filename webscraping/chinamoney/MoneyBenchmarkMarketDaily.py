...
DB_NAME = 'chinamoney'
TABLE_NAME = 'MoneyBenchmarkMarketDaily'

url = "http://www.chinamoney.com.cn/fe-c/moneyBenchmarkMarketDailySearchAction.do"
...
def insert_db_sql_values():
    # 引号坑死人.字段内不能加特殊符号 比如%
    sql = "insert into MoneyBenchmarkMarketDaily (date,`品种`,`最高利率`,`最低利率`,`加权利率`, `升降(基点)`," \
          "`成交金额(亿元)`,`增减(亿元)`)" \
...
def parse_html(html, save_row, insert_into):
    ...
    trs = html.find("td", attrs={"class": "mbr-title", "height": "28", "colspan": "7",
                                 "align": "left"}) \
